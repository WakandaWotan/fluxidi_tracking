...
class _CustomerBookingFlowState extends State<CustomerBookingFlow> {
  ...
  void initState() {
    super.initState();
    _entry = widget.entry;
    _lookup = widget.lookup ?? LimousinePlaceLookup();
    _quoteClient =
        widget.quoteClient ??
  ...
  Future<void> _geocodePickupIfNeeded() async {
    if (_pickupOwned &&
        !customerBookingPickupIsVacant(_pickup.value) &&
        _pickup.value.hasCoordinates) {
      return;
    }
  ...
  Future<void> _book(BookingPaymentSelection selection) async {
    ...
    try {
      ...
      unawaited(_persistBookedRide(stored));
    } catch (error) {
      if (!mounted) return;
      setState(() {
        _submitting = false;
            ...
            : LayoutBuilder(
                builder: (context, constraints) {
                  final wide = constraints.maxWidth >=
                      kCustomerBookingRouteWideBreakpoint;
                  final short = constraints.maxHeight < 520;
                  final form = _bookingFormFields(theme);
                  final map = _bookingMap();
                  final confirm = _confirmBar(compact: short);
                  ...
                  final airplane = _showAirplane && !short
                      ? Padding(
                      ...
                      Expanded(
                        child: CustomScrollView(
                          ...
                          slivers: [
                            SliverPadding(
                              padding: const EdgeInsets.fromLTRB(16, 8, 16, 0),
                              sliver: SliverToBoxAdapter(child: audience),
                            ),
                            if (_airportMode)
                              SliverPadding(
                                padding: const EdgeInsets.fromLTRB(16, 8, 16, 0),
                            ...
                            SliverPadding(
                              ...
                              sliver: SliverToBoxAdapter(
                                child: SizedBox(height: 280, child: map),
                              ),
                            ),
                            if (_tripSummaryLine.isNotEmpty)
                              SliverPadding(
                                padding: const EdgeInsets.fromLTRB(16, 0, 16, 8),
                                sliver: SliverToBoxAdapter(
                                  child: Text(
                                    _tripSummaryLine,
                                    key: kCustomerBookingTripSummaryKey,
  ...
  List<Widget> _bookingFormFields(ThemeData theme) {
    return [
      ...
      Text(_t(kCustomerBookingPickupSection), style: theme.textTheme.titleSmall),
      _whenToggle(),
      if (!_whenNow)
        ListTile(
          title: Text(_t(kCustomerBookingOutboundWhen)),
          subtitle: Text(_formatWhen(_pickupAt)),
          onTap: () => _pickDateTime(inbound: false),
        ),
  ...
  Widget _vehicleRow() {
    final categories = _bookableCategories;
    if (_vehiclesLoading && categories.isEmpty) {
      return Text(
        _t(kCustomerBookingVehiclesLoading),
        key: kCustomerBookingVehiclesLoadingKey,
      );
    }
    if (_vehiclesFailed && categories.isEmpty) {
      return Text(
    ...
    return Column(
      ...
      children: [
          ...
          children: [
            for (final category in categories)
              SizedBox(
                width: 148,
                child: InkWell(
                  key: customerBookingVehicleKey(
                    companyPlanVehicleCategoryWire(category),
                  ),
                  onTap: () {
                    setState(() {
                  ...
                  child: DecoratedBox(
                    decoration: BoxDecoration(
                      color: _palette.surface,
                      border: Border.all(
                        color: _vehicleCategory == category
                            ? _palette.gold
                            : _palette.border,
                        width: _vehicleCategory == category ? 2 : 1.2,
                      ),
                      borderRadius: BorderRadius.circular(12),
                    ),
                    child: Column(
                      children: [
                        _vehicleImage(category),
                        Text(
                          companyPlanVehicleCategoryLabel(category, _language),
                          style: TextStyle(
                            color: _palette.textPrimary,
                            fontWeight: FontWeight.w700,
                          ),
                        ...
                        Text(
                          customerBookingVehicleCapacityLabel(
                            vehicle: customerBookingExampleVehicleForCategory(
        ...
        Padding(
          ...
          child: Text(
            _t(kCustomerBookingVehicleSampleHint),
            key: kCustomerBookingVehicleHintKey,
  ...
  Widget _vehicleImage(CompanyPlanVehicleCategory category) {
    final example = customerBookingExampleVehicleForCategory(
      vehicles: _companyVehicles,
      category: category,
      passengers: _passengers,
    );
    final photo = customerBookingVehiclePhotoUrl(example);
    if (photo.isNotEmpty) {
