...
class _CustomerBookingsPageState extends State<CustomerBookingsPage> {
  ...
  // refresh from the authoritative record. The displayed token is never
  // upgraded locally: only a verified provider settlement makes a booking paid.
  String _t({
    required String nl,
    required String en,
    required String fr,
  ...
  void _onPendingPaymentNotifierChangedForCustomerList() {
    ...
    // Dossier 02: never patch the local entry to paid. The authoritative
    // record decides, so a return from checkout only triggers a refresh.
    debugPrint(
      '[PAYMENT_RETURN][CUSTOMER_REFRESH] surface=list booking=${_safeRefPreview(canonicalId)} paymentBooking=${_safeRefPreview(paymentBookingIdHit)} status=${pending.status.name} local_patch=none',
    );
    if (!_refreshing) {
  ...
  Widget _roundtripLegCard({
    ...
    if (leg.isCancelled) {
      statusColor = const Color(0xFFE88989);
    } else if (leg.isCompleted) {
      statusColor = const Color(0xFF66D9A8);
    } else {
      statusColor = const Color(0xFF34D29A);
    }
    final showCancelLeg =
          ...
          if (roundtripLegs.isNotEmpty) ...[
            ...
            ...roundtripLegs.map(
              (leg) => _roundtripLegCard(
                ...
                bookingKnownPaid: paymentKnownPaid,
              ),
            ),
          ],
          const SizedBox(height: 10),
