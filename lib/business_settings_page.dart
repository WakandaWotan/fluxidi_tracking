...
class _BusinessSettingsPageState extends State<BusinessSettingsPage> {
  ...
  bool _hasReadinessValue(String value) => value.trim().isNotEmpty;

  Widget _chironReadinessRow({
  ...
  Widget _chironReadinessSection(String title, List<Widget> children) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(
  ...
  Widget _buildChironReadinessCard() {
        ...
        return ValueListenableBuilder<List<DriverProfile>>(
          ...
          builder: (context, drivers, __) {
            ...
            final vehiclesWithPlate = visibleVehicles
                .where((v) => _hasReadinessValue(v.licensePlate))
                .length;
            final vehiclesWithExploitationLicense = visibleVehicles
                .where((v) => _hasReadinessValue(v.exploitationLicenseNumber))
                .length;
            final vehiclesWithRegistration = visibleVehicles
                .where((v) => _hasReadinessValue(v.vehicleRegistrationNumber))
                .length;
            ...
            final activeDriverCount = visibleDrivers
                .where((d) => d.isActive)
                .length;
            final driversWithTaxiCardNumber = visibleDrivers
                .where((d) => _hasReadinessValue(d.taxiDriverCardNumber))
                .length;
            final driversWithTaxiCardExpiry = visibleDrivers
                .where((d) => _hasReadinessValue(d.taxiDriverCardExpiry))
                .length;

            final companyNameReady =
                _hasReadinessValue(_backendLegalNameCtrl.text) ||
                _hasReadinessValue(_backendCompanyNameCtrl.text);
            final kboReady = _hasReadinessValue(_backendRegistrationCtrl.text);
            final vatReady = _hasReadinessValue(_backendVatNumberCtrl.text);
            final addressReady =
                _hasReadinessValue(_backendAddressCtrl.text) &&
                _hasReadinessValue(_backendPostcodeCtrl.text) &&
                _hasReadinessValue(_backendCityCtrl.text);

            String countText(int count) => count.toString();
            String ratioText(int ready, int total) => '$ready/$total';
            ...
            return _card(
              ...
              child: Column(
                ...
                children: [
                  Text(
                    _t(
                      nl: 'Deze checklist blokkeert niets. Ze helpt je voorbereiden op een toekomstige Chiron-koppeling.',
                      en: 'This checklist does not block anything. It helps prepare for a future Chiron integration.',
                      fr: 'Cette checklist ne bloque rien. Elle aide à préparer une future intégration Chiron.',
                  ...
                  _chironReadinessSection(
                    ...
                    [
                      ...
                      _chironReadinessItem(
                        ready:
                            vehicleCount > 0 &&
                            vehiclesWithPlate == vehicleCount,
                        label: _t(
                          nl: 'Met nummerplaat',
                      ...
                      _chironReadinessItem(
                        ready:
                            vehicleCount > 0 &&
                            vehiclesWithExploitationLicense == vehicleCount,
                        label: _t(
                  ...
                  _chironReadinessSection(
                    ...
                    [
                      _chironReadinessItem(
                        ready: driverCount > 0,
                        label: _t(
                          nl: 'Aantal chauffeurs',
                          en: 'Total drivers',
                          fr: 'Nombre de chauffeurs',
                          es: 'Total conductores',
                        ),
                        value: countText(driverCount),
                      ),
                      _chironReadinessItem(
                        ...
                        value: countText(activeDriverCount),
                      ),
                      _chironReadinessItem(
                        ready:
                            driverCount > 0 &&
                            driversWithTaxiCardNumber == driverCount,
                        label: _t(
