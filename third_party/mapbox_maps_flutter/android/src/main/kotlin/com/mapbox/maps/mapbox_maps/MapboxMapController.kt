...
class MapboxMapController(
  ...
  */
  ...
  init {
    ...
    // FLUXIDI Phase 2A local patch: expose this MapView to companion plugins
    // through a per-controller registry keyed by the plugin's own
    // channelSuffix. See third_party/mapbox_maps_flutter/PATCHES.md.
  ...
  override fun dispose() {
    // FLUXIDI Phase 2A local patch: unregister BEFORE the mapView == null
    // early-return so a duplicate dispose call is still a no-op and every
    // valid dispose deterministically notifies companion plugins to tear
    // down their custom LocationProvider on this exact instance. See
    // third_party/mapbox_maps_flutter/PATCHES.md.
