 ...
 *
 * Indexed-company fast path: one targeted code-membership GET, zero writes,
 * and no page/manifest IO. Repair writes pages first and the code key only
...
export function emptyRegistryOps() {
  return {
    code_gets: 0,
    code_puts: 0,
    page_gets: 0,
...
function tallyWrite(ops) {
  ops.writes = (ops.code_puts || 0) + (ops.page_puts || 0) + (ops.manifest_puts || 0);
  return ops;
}
...
export function isIndexedRegistryEntry(existing, companyCode) {
  if (!existing || typeof existing !== "object" || Array.isArray(existing)) return false;
  return String(existing.company_code || "").trim() === String(companyCode || "").trim();
}

export async function readRegistrySnapshot(kv, ops = null) {
  if (ops) ops.manifest_gets += 1;
...
export async function writeRegistrySnapshot(kv, snapshot, writeToken, ops = null) {
  const manifest = { ...snapshot.manifest, write_token: writeToken };
  for (const page of snapshot.pages || []) {
    if (ops) ops.page_puts += 1;
...
export async function upsertCompanyRegistryEntry(kv, input, {
  ...
  if (!kv || typeof kv.get !== "function" || typeof kv.put !== "function") {
    return { ok: false, error: "kv_unbound", already_indexed: false, ops: tallyWrite(ops) };
  }
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    ...
    if (confirmOk(before, intended, confirm, entry, token)) {
      ops.code_puts += 1;
      await kv.put(codeKey, JSON.stringify(entry));
      return {
        ok: true,
  ...
  return {
    ok: false,
    error: "registry_concurrent_update",
    already_indexed: false,
    ops: tallyWrite(ops),
  };
}
...
export function createMemoryRegistryKv(seed = {}) {
  ...
  return {
    ...
    async list() {
      counts.lists.push("*");
      throw new Error("company_registry_index_forbids_list");
    },
  };
}
