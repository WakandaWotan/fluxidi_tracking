...
test("4/5/6) modified IV, modified ciphertext and truncated tokens are rejected", async () => {
  ...
  const flip = (segment) => `${segment.slice(0, -1)}${segment.endsWith("A") ? "B" : "A"}`;
...
test("20) every limousine gate remains OFF", () => {
  ...
  assert.ok(wrangler.includes("limqs1.<iv>.<ciphertext>"));
  assert.ok(wrangler.includes("The previous HMAC form is rejected"));
});
