// COMPANY-CUSTOMER-OPS-P0 — local Worker host for Klantenbeheer / offertes.
//
...
const COMPANY_ID = "demo_company_p0";
const SESSION_TOKEN = "cst_local_demo_synthetic";
...
async function seedSession(kv) {
  const hash = await sha256Hex(SESSION_TOKEN);
  ...
  await kv.put(
    ...
    JSON.stringify({
      ...
      company_code: "FLX-DEMO1",
      company_display_name: "Fluxidi Demo Cars",
      expires_at: expires,
    }),
  );
}
...
async function main() {
  ...
  server.listen(PORT, HOST, () => {
    ...
    console.log(`[local-demo] company ${COMPANY_ID} token ${SESSION_TOKEN}`);
  });
}
